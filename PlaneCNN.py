...
class CustomDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        self.image_paths = sorted([os.path.join(image_dir, filename) for filename in os.listdir(image_dir)])
        self.label_paths = sorted([os.path.join(label_dir, filename) for filename in os.listdir(label_dir)])
    ...
    def __getitem__(self, idx):
        ...
        image = read_image(image_path)  # Use appropriate function to read images
        label = int(np.loadtxt(label_path))  # Load label from text file
...
train_images_dir = "Airplane detection.v2i.yolov7pytorch/test/images"
train_labels_dir = "Airplane detection.v2i.yolov7pytorch/test/labels"
test_images_dir = "Airplane detection.v2i.yolov7pytorch/train/images"
test_labels_dir = "Airplane detection.v2i.yolov7pytorch/train/labels"
val_images_dir = "Airplane detection.v2i.yolov7pytorch/valid/images"
val_labels_dir = "Airplane detection.v2i.yolov7pytorch/valid/labels"

# Create dataset instances
train_dataset = CustomDataset(root=train_images_dir, labels_dir=train_labels_dir, transform=ToTensor())
val_dataset = CustomDataset(root=val_images_dir, labels_dir=val_labels_dir, transform=ToTensor())
test_dataset = CustomDataset(root=test_images_dir, labels_dir=test_labels_dir, transform=ToTensor())
